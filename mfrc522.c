 ...
 */
void PCD_Init(MFRC522Ptr_t mfrc, spi_inst_t *spi, uint csPin) {

	mfrc->spi = spi0;
	mfrc->_chipSelectPin = csPin;
	gpio_put(RESET_PIN, 0);
    sleep_ms(1000);
    gpio_put(RESET_PIN, 1);
	sleep_ms(50);

    gpio_init(mfrc->_chipSelectPin);
    gpio_set_dir(mfrc->_chipSelectPin, GPIO_OUT);
...
/**
 * Health check function for scanner connectivity.
 * Attempts to read a register from the MFRC522 with timeout.
 * If successful, scanner is considered healthy.
 * Returns true if scanner is responding, false if disconnected or timeout.
 */
bool PCD_CheckHealth(MFRC522Ptr_t mfrc) {
	// Try to read the VersionReg to check if chip is alive
	// VersionReg should always be readable if the chip is responding
	uint32_t start_time = time_us_32();
	uint32_t timeout_us = SCANNER_HEALTH_CHECK_TIMEOUT_MS * 1000;
	
	while ((time_us_32() - start_time) < timeout_us) {
		uint8_t version = PCD_ReadRegister(mfrc, VersionReg);
		// A valid MFRC522 version register should not be 0x00 or 0xFF
		// Valid values are typically 0x91 or 0x92 for different versions
